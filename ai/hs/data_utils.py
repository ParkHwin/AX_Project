...
def load_labeled_data(pkl_path=config.PKL_PATH):
    """
    LSWMD.pkl(원본, unlabeled 포함) 또는 preprocess_pkl.py로 미리 걸러둔
    LSWMD_labeled_only.pkl(clean_label 컬럼 이미 존재) 둘 다 지원.
    """
    print("=== 1. 데이터 로드 및 정제 ===")
    data = pd.read_pickle(pkl_path)

    if 'clean_label' in data.columns:
        print("-> 이미 전처리된 pkl(clean_label 존재)로 감지 -> 별도 필터링 없이 그대로 사용")
