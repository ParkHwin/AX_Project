...
- **test_image**: `image_num`(PK), `user_num`(FK→user, CASCADE), `image`(LONGBLOB), `image_name`(업로드 파일 이름, NULL 허용), `time`(자동)
- **result**: `result_num`(PK), `user_num`(FK→user), `image_num`(FK→test_image), `class_id1~3`(SMALLINT 코드), `class_name1~3`(VARCHAR 클래스 이름), `confidence1~3`(FLOAT 확률), `image_name`(원본 파일 이름), `detime`(자동)
  ...
  `class_name1~3`과 `image_name`은 클라이언트가 보내지 않는다 — 서버가 `class_id`를
  [label_mapping.json](label_mapping.json)으로 변환하고, `image_name`은 해당 `test_image` 행에서 복사해 채운다.
